...
@app.before_request
def _check_auth():
    # 0. 以 BILI_DISCLAIMER_SKIP 启动时（插件拉起面板时固定设置），视为已确认免责声明，
    #    直接跳过网页端「我同意」门槛，避免用户卡在免责声明页无法跳转。
    skip_disclaimer = bool(os.getenv('BILI_DISCLAIMER_SKIP'))
    # 1. 先检查免责声明
    if not skip_disclaimer and not session.get('disclaimer_agreed'):
